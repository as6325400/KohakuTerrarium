 ...
 **Terrarium** 是托管行程内所有运行中 Creature 的运行时引擎。一只独立的 Agent 就是引擎里的 1-creature graph；多 Agent 团队则是用频道连起来的 connected graph。引擎负责生命周期、共用频道、热插拔、输出接线、以及在拓扑变更时跟著走的 session 合并 / 分裂记账。它本身不含 LLM、不做决策 — 纯粹接线。Creature 本身不知道自己在 Terrarium 里 — 它们只知道自己 listen 哪些频道名字、能送到哪些频道名字，而引擎让那些名字变成真的。
...
async with Terrarium() as engine:
    ...
    # tester 落在自己的 singleton graph；connect() 会把它合进来。
    swe = engine["swe"]
    result = await engine.connect(swe, tester, channel="review")
    # result.delta_kind == "merge"
```

跨 graph 的 `connect()` 会合并两个 graph — environment 取联集，挂著的 session store 合成一份，新的 listener 会被注入 `ChannelTrigger`。`disconnect()` 可能把 graph 拆回两边、并把 parent session 复制到两侧。参考 [`examples/code/terrarium_hotplug.py`](../../examples/code/terrarium_hotplug.py)。
...
更多写法 (事件订阅、热插拔、独立 + recipe 共存) 看 [程序化使用指南](programmatic-usage.md)，以及 [`examples/code/`](../../examples/code/) 里可执行的脚本 (`terrarium_solo.py`、`terrarium_recipe.py`、`terrarium_hotplug.py`)。

旧版 `TerrariumRuntime` 在过渡期间还留在硬盘上；新代码请用 `Terrarium`。
