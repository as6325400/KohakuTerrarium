...
托管行程内所有运行中Creature的运行时引擎。一只独立的 Agent 就是引擎里的 1-creature graph；多 Agent 团队则是用频道连起来的 connected graph。引擎拥有 creature CRUD、channel CRUD、热插拔、输出接线、以及在拓扑变更时跟着走的 session 合并 / 分裂记账。它不含 LLM、不做决策 — 纯粹接线。Creature 不知道自己在 Terrarium 里；它们仍然可以独立执行。完整说明：[Terrarium](multi-agent/terrarium.md)。

## Graph / 图

Terrarium 引擎里的一个连通分量：透过频道连起来的一组 Creature。两只无关的 Creature 各自处于不同的 graph；在它们之间画一条频道会把两个 graph 合并 (并合并两边的 session 历史)。把两半之间最后一条频道拔掉则会把 graph 分裂 (并把历史复制到两边)。Graph 是 session 的单位 — 同一个 graph 里的 Creature 共用同一个 `.kohakutr` 文件。完整说明：[Terrarium](multi-agent/terrarium.md)。
