---
title: 触发器 (Trigger)
summary: 任何在没有明确用户输入时唤醒控制器的东西——计时器、上下文更新、频道，以及自定义 watcher。
tags:
  - concepts
  - module
  - trigger
...
**触发器 (trigger)** 是任何在没有明确用户输入时唤醒控制器的东西。计时器、上下文更新 watcher、频道监听器，以及自定义监看条件，全部都属于 trigger。每个 trigger 都会作为背景作业执行，并在触发条件成立时把 `TriggerEvent` 推进事件队列。
...
- **`timer`**—— 每 N 秒触发一次。
- **`context`**—— 在 debounced 的上下文更新后触发。
- **`channel`**—— 监听具名频道；收到消息时触发。
- **`custom` / `package`**—— 从模块载入你自己的 trigger 类别。

框架也内建了时钟对齐的 `SchedulerTrigger`，但它是以 setup tool `add_schedule` 的形式暴露，而不是 `triggers:` 里的配置期类型。
...
- **由上下文驱动的摘要**。 `context` trigger 可以对快速更新做 debounce，等共享状态稳定后再派遣 `summarize` 子 Agent。
- **运行时排程**。 `add_schedule` setup tool 让 Agent 能在运行时安装时钟对齐的重复唤醒，而不必把它硬编码进 `triggers:`。
