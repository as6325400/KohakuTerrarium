...
托管行程內所有運行中生物的執行期引擎。一隻獨立 agent 就是引擎裡的 1-creature graph；多代理團隊則是用頻道連起來的 connected graph。引擎擁有 creature CRUD、channel CRUD、熱插拔、輸出接線、以及在拓樸變更時跟著走的 session 合併 / 分裂記帳。它不含 LLM、不做決策 — 純粹接線。生物不知道自己在生態瓶裡；它們仍然可以獨立執行。完整說明：[生態瓶](multi-agent/terrarium.md)。

## Graph / 圖

生態瓶引擎裡的一個連通分量：透過頻道連起來的一組生物。兩隻無關的生物各自處於不同的 graph；在它們之間畫一條頻道會把兩個 graph 合併 (並合併兩邊的 session 歷史)。把兩半之間最後一條頻道拔掉則會把 graph 分裂 (並把歷史複製到兩邊)。Graph 是 session 的單位 — 同一個 graph 裡的生物共用同一個 `.kohakutr` 檔案。完整說明：[生態瓶](multi-agent/terrarium.md)。
