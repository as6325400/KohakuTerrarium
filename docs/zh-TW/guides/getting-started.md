...
你會進入 SWE agent 的互動式提示環境。輸入一個請求後，它會在目前工作目錄中使用 shell、檔案與編輯工具。Ctrl+C 可乾淨結束，並印出恢復提示。
...
- **`ModuleNotFoundError: pywebview`。** `kt app` 需要桌面額外依賴：`pip install 'kohakuterrarium[full]'`（或改用 `kt web`）。
